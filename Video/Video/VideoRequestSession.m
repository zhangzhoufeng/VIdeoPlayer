...
@interface VideoRequestSession()<VideoRequestTaskDelegate>
...
@implementation VideoRequestSession
...
- (BOOL)respondWithDataForRequest:(AVAssetResourceLoadingDataRequest *)dataRequest
{
    ...
    if ((self.task.offset +self.task.videoDownLoadOffset) < startOffset)
    {
    ...
    NSUInteger unreadBytes = self.task.videoDownLoadOffset - ((NSInteger)startOffset - self.task.offset);
    ...
    BOOL didRespondFully = (self.task.offset + self.task.videoDownLoadOffset) >= endOffset;
...
- (void)dealWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    if (self.task.videoDownLoadOffset > 0) {
    ...
    if (!self.task) {
        ...
        [self.task setVideoUrl:interceptedURL offset:0];
    } else {
        // 如果新的rang的起始位置比当前缓存的位置还大300k，则重新按照range请求数据
        if (self.task.offset + self.task.videoDownLoadOffset + 1024 * 300 < range.location ||
            // 如果往回拖也重新请求
            range.location < self.task.offset) {
            [self.task setVideoUrl:interceptedURL offset:range.location];
        }
    }
